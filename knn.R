...
kNearestAlgorithim <- function(data,k,factor){
  ...
  for(j in 1:nrow(test1_set)){
    ...
    label=as.numeric(names(sort(summary(as.factor(c(newOrder[1:k,ncol(newOrder)-1]))), decreasing=T)[1]))
...
PercentageOfCorrectLabels = round(kNearestAlgorithim(USPS,1,'euclidean'));

Accuracy<-0.0
for(k in 1:9){
  Accuracy[k]<-kNearestAlgorithim(USPS,k,'tangent');
}
plot(1:9,Accuracy, type = "b", xlab = "k", ylab = "Accuracy", main = "KNN (Tangent Distance) on USPSsubset Data")

Accuracy<-0.0
for(k in 1:9){
  Accuracy[k]<-kNearestAlgorithim(USPS,k,'euclidean');
}
plot(1:9,Accuracy, type = "b", xlab = "k", ylab = "Accuracy", main = "KNN (Euclidean Distance) on USPSsubset Data")
